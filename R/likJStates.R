...
#' @export
#' @return A matrix with 9 rows and columns equal to the number of markers.
#' Element (i,j) is the likelihood conditioned on Jacquard state i for marker j.
#' @examples
#' p = c(0.1, 0.2, 0.3, 0.4)
#' a  = c(1,1)
#' b  = c(2,2)
...
#' l2 = likJStates(a,b,cc,d, pa,pb,pc,pd)

likJStates = function(a,b,cc,d, pa, pb, pc, pd){
  I12 = a==b
  I13 = a==cc
  I14 = a==d
  I23 = b==cc
  ...
  for(s in 1:9){
    Delta[s] = 1
    liks[[s]] = as.numeric(PG %*% Delta)
    Delta[s] = 0
  }
  matrix(byrow = TRUE, nrow = 9, unlist(liks))
}
