...
#'                             method = "parametric", seed = seed, level = 0.95, bca = bca)
#' resNonparametric = realisedPhi(ped = ped, ids = ids, N = N, B = B,
#'                             method = "nonparametric", seed = seed, level = 0.95, bca = bca)
...
realisedPhi <- function(ped, ids = NULL, N = 2, B = 2,
                        seed = NULL, level = 0.95, method = "parametric", bca = FALSE){
  if(!is.null(seed))
    set.seed(seed)
  n = nMarkers(ped)
  ...
  for (j in 1:N){
      ...
      boot1 = ibdBootstrap(sim, ids = ids, kappa = kappa.realised[[j]],  N = B, plot = F, method = method)
